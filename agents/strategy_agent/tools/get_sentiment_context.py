...
Per-profile denial stub overrides the global tools/get_sentiment_context.py
via subagents.get_paths() priority order: agents/<profile>/tools/<name>.py
wins over tools/<name>.py.
"""
from helpers.tool import Tool, Response
class GetSentimentContext(Tool):
    async def execute(self, **kwargs) -> Response:
        msg = (
            "Tool 'get_sentiment_context' is NOT AVAILABLE to strategy_agent (hard-scoped "
