...
Per-profile denial stub overrides the global tools/get_trade_context.py via
subagents.get_paths() priority order: agents/<profile>/tools/<name>.py wins
over tools/<name>.py.
"""
from helpers.tool import Tool, Response
class GetTradeContext(Tool):
    async def execute(self, **kwargs) -> Response:
        msg = (
            "Tool 'get_trade_context' is NOT AVAILABLE to strategy_agent (hard-scoped "
