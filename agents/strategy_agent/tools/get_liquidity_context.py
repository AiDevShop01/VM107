...
Per-profile denial stub overrides the global tools/get_liquidity_context.py
via subagents.get_paths() priority order: agents/<profile>/tools/<name>.py
wins over tools/<name>.py.
"""
from helpers.tool import Tool, Response
class GetLiquidityContext(Tool):
    async def execute(self, **kwargs) -> Response:
        msg = (
            "Tool 'get_liquidity_context' is NOT AVAILABLE to strategy_agent (hard-scoped "
