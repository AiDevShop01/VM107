"""Phase 95-12 — ManufacturingDomainAnalyst specialist (narrative-only).

EXPLAINS the Manufacturing Domain (one of the 12 canonical macro domains —
...
class ManufacturingDomainAnalyst:
    """Specialist analyst for the Manufacturing Domain."""

    DOMAIN = "Manufacturing"
    DOMAIN_SLUG = "manufacturing"
    AGENT_ID = "vm107.manufacturing_domain_analyst"

    def invoke(
        self, domain: Domain, context: dict | None = None
    ) -> SpecialistResponse:
        assert domain.slug == self.DOMAIN_SLUG, (
            f"ManufacturingDomainAnalyst received domain.slug={domain.slug!r} — "
