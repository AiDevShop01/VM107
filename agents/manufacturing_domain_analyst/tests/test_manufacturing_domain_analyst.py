"""Phase 95-12 — ManufacturingDomainAnalyst specialist contract tests."""
...
def _make_domain(
    *,
    slug: str = "manufacturing",
    title: str = "Manufacturing",
...
def test_returns_specialist_response_shape():
    from agents.manufacturing_domain_analyst import ManufacturingDomainAnalyst

    domain = _make_domain()
    resp = ManufacturingDomainAnalyst().invoke(domain)
...
def test_never_recomputes_score():
    ...
    from agents.manufacturing_domain_analyst import agent as mod
    ...
    for needle in banned:
        assert needle not in src, (
            f"banned import/symbol in manufacturing_domain_analyst: {needle}"
        )
    tree = ast.parse(src)
    for node in ast.walk(tree):
        if isinstance(node, ast.Attribute) and node.attr in {
            ...
            pytest.fail(
                f"manufacturing_domain_analyst calls {node.attr} — violates "
...
def test_slug_mismatch_raises():
    ...
    with pytest.raises(AssertionError):
        ManufacturingDomainAnalyst().invoke(domain)
