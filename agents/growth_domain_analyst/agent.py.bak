"""Phase 95-12 — GrowthDomainAnalyst specialist (narrative-only).

EXPLAINS the Growth Domain (one of the 12 canonical macro domains —
...
class GrowthDomainAnalyst:
    """Specialist analyst for the Growth Domain."""

    DOMAIN = "Growth"
    DOMAIN_SLUG = "growth"
    AGENT_ID = "vm107.growth_domain_analyst"

    def invoke(
        self, domain: Domain, context: dict | None = None
    ) -> SpecialistResponse:
        assert domain.slug == self.DOMAIN_SLUG, (
            f"GrowthDomainAnalyst received domain.slug={domain.slug!r} — "
