"""Phase 95-12 — MonetaryPolicyDomainAnalyst specialist contract tests."""
...
def _make_domain(
    *,
    slug: str = "monetary_policy",
    title: str = "Monetary Policy",
...
def test_returns_specialist_response_shape():
    from agents.monetary_policy_domain_analyst import MonetaryPolicyDomainAnalyst

    domain = _make_domain()
    resp = MonetaryPolicyDomainAnalyst().invoke(domain)
...
def test_never_recomputes_score():
    ...
    from agents.monetary_policy_domain_analyst import agent as mod
    ...
    for needle in banned:
        assert needle not in src, (
            f"banned import/symbol in monetary_policy_domain_analyst: {needle}"
        )
    tree = ast.parse(src)
    for node in ast.walk(tree):
        if isinstance(node, ast.Attribute) and node.attr in {
            ...
            pytest.fail(
                f"monetary_policy_domain_analyst calls {node.attr} — violates "
...
def test_slug_mismatch_raises():
    ...
    with pytest.raises(AssertionError):
        MonetaryPolicyDomainAnalyst().invoke(domain)
