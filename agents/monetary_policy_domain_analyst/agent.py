"""Phase 95-12 — MonetaryPolicyDomainAnalyst specialist (narrative-only).

EXPLAINS the Monetary Policy Domain (one of the 12 canonical macro domains —
...
class MonetaryPolicyDomainAnalyst:
    """Specialist analyst for the Monetary Policy Domain."""

    DOMAIN = "Monetary Policy"
    DOMAIN_SLUG = "monetary_policy"
    AGENT_ID = "vm107.monetary_policy_domain_analyst"

    def invoke(
        self, domain: Domain, context: dict | None = None
    ) -> SpecialistResponse:
        assert domain.slug == self.DOMAIN_SLUG, (
            f"MonetaryPolicyDomainAnalyst received domain.slug={domain.slug!r} — "
