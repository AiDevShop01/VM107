"""Phase 95-12 — ConsumerDomainAnalyst specialist (narrative-only).

EXPLAINS the Consumer Domain (one of the 12 canonical macro domains —
...
class ConsumerDomainAnalyst:
    """Specialist analyst for the Consumer Domain."""

    DOMAIN = "Consumer"
    DOMAIN_SLUG = "consumer"
    AGENT_ID = "vm107.consumer_domain_analyst"

    def invoke(
        self, domain: Domain, context: dict | None = None
    ) -> SpecialistResponse:
        assert domain.slug == self.DOMAIN_SLUG, (
            f"ConsumerDomainAnalyst received domain.slug={domain.slug!r} — "
