"""Phase 95-12 — CommoditiesDomainAnalyst specialist contract tests."""
...
def _make_domain(
    *,
    slug: str = "commodities",
    title: str = "Commodities",
...
def test_returns_specialist_response_shape():
    from agents.commodities_domain_analyst import CommoditiesDomainAnalyst

    domain = _make_domain()
    resp = CommoditiesDomainAnalyst().invoke(domain)
...
def test_never_recomputes_score():
    ...
    from agents.commodities_domain_analyst import agent as mod
    ...
    for needle in banned:
        assert needle not in src, (
            f"banned import/symbol in commodities_domain_analyst: {needle}"
        )
    tree = ast.parse(src)
    for node in ast.walk(tree):
        if isinstance(node, ast.Attribute) and node.attr in {
            ...
            pytest.fail(
                f"commodities_domain_analyst calls {node.attr} — violates "
...
def test_slug_mismatch_raises():
    ...
    with pytest.raises(AssertionError):
        CommoditiesDomainAnalyst().invoke(domain)
