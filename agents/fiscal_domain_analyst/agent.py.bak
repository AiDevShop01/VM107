"""Phase 95-12 — FiscalDomainAnalyst specialist (narrative-only).

EXPLAINS the Fiscal Domain (one of the 12 canonical macro domains —
...
class FiscalDomainAnalyst:
    """Specialist analyst for the Fiscal Domain."""

    DOMAIN = "Fiscal"
    DOMAIN_SLUG = "fiscal"
    AGENT_ID = "vm107.fiscal_domain_analyst"

    def invoke(
        self, domain: Domain, context: dict | None = None
    ) -> SpecialistResponse:
        assert domain.slug == self.DOMAIN_SLUG, (
            f"FiscalDomainAnalyst received domain.slug={domain.slug!r} — "
