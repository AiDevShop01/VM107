"""Phase 95-12 — FiscalDomainAnalyst specialist contract tests."""
...
def test_returns_specialist_response_shape():
    from agents.fiscal_domain_analyst import FiscalDomainAnalyst

    domain = _make_domain()
    resp = FiscalDomainAnalyst().invoke(domain)
...
def test_never_recomputes_score():
    ...
    from agents.fiscal_domain_analyst import agent as mod
    ...
    for needle in banned:
        assert needle not in src, (
            f"banned import/symbol in fiscal_domain_analyst: {needle}"
        )
    tree = ast.parse(src)
    for node in ast.walk(tree):
        if isinstance(node, ast.Attribute) and node.attr in {
            ...
            pytest.fail(
                f"fiscal_domain_analyst calls {node.attr} — violates "
...
def test_slug_mismatch_raises():
    ...
    with pytest.raises(AssertionError):
        FiscalDomainAnalyst().invoke(domain)
