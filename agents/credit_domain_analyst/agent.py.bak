"""Phase 95-12 — CreditDomainAnalyst specialist (narrative-only).

EXPLAINS the Credit Domain (one of the 12 canonical macro domains —
...
class CreditDomainAnalyst:
    """Specialist analyst for the Credit Domain."""

    DOMAIN = "Credit"
    DOMAIN_SLUG = "credit"
    AGENT_ID = "vm107.credit_domain_analyst"

    def invoke(
        self, domain: Domain, context: dict | None = None
    ) -> SpecialistResponse:
        assert domain.slug == self.DOMAIN_SLUG, (
            f"CreditDomainAnalyst received domain.slug={domain.slug!r} — "
