"""Phase 95-12 — InflationDomainAnalyst specialist contract tests."""
...
def test_returns_specialist_response_shape():
    from agents.inflation_domain_analyst import InflationDomainAnalyst

    domain = _make_domain()
    resp = InflationDomainAnalyst().invoke(domain)
...
def test_never_recomputes_score():
    ...
    from agents.inflation_domain_analyst import agent as mod
    ...
    for needle in banned:
        assert needle not in src, (
            f"banned import/symbol in inflation_domain_analyst: {needle}"
        )
    tree = ast.parse(src)
    for node in ast.walk(tree):
        if isinstance(node, ast.Attribute) and node.attr in {
            ...
            pytest.fail(
                f"inflation_domain_analyst calls {node.attr} — violates "
...
def test_slug_mismatch_raises():
    ...
    with pytest.raises(AssertionError):
        InflationDomainAnalyst().invoke(domain)
