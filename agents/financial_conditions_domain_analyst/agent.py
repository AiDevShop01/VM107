"""Phase 95-12 — FinancialConditionsDomainAnalyst specialist (narrative-only).

EXPLAINS the Financial Conditions Domain (one of the 12 canonical macro domains —
...
class FinancialConditionsDomainAnalyst:
    """Specialist analyst for the Financial Conditions Domain."""

    DOMAIN = "Financial Conditions"
    DOMAIN_SLUG = "financial_conditions"
    AGENT_ID = "vm107.financial_conditions_domain_analyst"

    def invoke(
        self, domain: Domain, context: dict | None = None
    ) -> SpecialistResponse:
        assert domain.slug == self.DOMAIN_SLUG, (
            f"FinancialConditionsDomainAnalyst received domain.slug={domain.slug!r} — "
