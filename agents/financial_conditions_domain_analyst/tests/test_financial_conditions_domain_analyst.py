"""Phase 95-12 — FinancialConditionsDomainAnalyst specialist contract tests."""
...
def _make_domain(
    *,
    slug: str = "financial_conditions",
    title: str = "Financial Conditions",
...
def test_returns_specialist_response_shape():
    from agents.financial_conditions_domain_analyst import FinancialConditionsDomainAnalyst

    domain = _make_domain()
    resp = FinancialConditionsDomainAnalyst().invoke(domain)
...
def test_never_recomputes_score():
    ...
    from agents.financial_conditions_domain_analyst import agent as mod
    ...
    for needle in banned:
        assert needle not in src, (
            f"banned import/symbol in financial_conditions_domain_analyst: {needle}"
        )
    tree = ast.parse(src)
    for node in ast.walk(tree):
        if isinstance(node, ast.Attribute) and node.attr in {
            ...
            pytest.fail(
                f"financial_conditions_domain_analyst calls {node.attr} — violates "
...
def test_slug_mismatch_raises():
    ...
    with pytest.raises(AssertionError):
        FinancialConditionsDomainAnalyst().invoke(domain)
