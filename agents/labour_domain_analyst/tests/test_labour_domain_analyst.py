"""Phase 95-12 — LabourDomainAnalyst specialist contract tests."""
...
def test_returns_specialist_response_shape():
    from agents.labour_domain_analyst import LabourDomainAnalyst

    domain = _make_domain()
    resp = LabourDomainAnalyst().invoke(domain)
...
def test_never_recomputes_score():
    ...
    from agents.labour_domain_analyst import agent as mod
    ...
    for needle in banned:
        assert needle not in src, (
            f"banned import/symbol in labour_domain_analyst: {needle}"
        )
    tree = ast.parse(src)
    for node in ast.walk(tree):
        if isinstance(node, ast.Attribute) and node.attr in {
            ...
            pytest.fail(
                f"labour_domain_analyst calls {node.attr} — violates "
...
def test_slug_mismatch_raises():
    ...
    with pytest.raises(AssertionError):
        LabourDomainAnalyst().invoke(domain)
