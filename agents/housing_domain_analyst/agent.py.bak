"""Phase 95-12 — HousingDomainAnalyst specialist (narrative-only).

EXPLAINS the Housing Domain (one of the 12 canonical macro domains —
...
class HousingDomainAnalyst:
    """Specialist analyst for the Housing Domain."""

    DOMAIN = "Housing"
    DOMAIN_SLUG = "housing"
    AGENT_ID = "vm107.housing_domain_analyst"

    def invoke(
        self, domain: Domain, context: dict | None = None
    ) -> SpecialistResponse:
        assert domain.slug == self.DOMAIN_SLUG, (
            f"HousingDomainAnalyst received domain.slug={domain.slug!r} — "
