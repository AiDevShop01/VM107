"""Phase 48 Plan 08b — E2E golden: BUDGET_EXCEEDED_LOOP. REQ-48-E2E-GOLDEN-BUDGET-LOOP.

Pre-arms budget snapshot so preflight() returns False (loop_total_usd
near per-loop cap → not enough remaining for one more iteration).
Preflight runs BEFORE Critic on iter 0 → loop terminates immediately
with BUDGET_EXCEEDED_LOOP.
"""
from __future__ import annotations

from core.agents.refinement_orchestrator import run_refinement_loop
from core.agents.refinement_orchestrator.policy_constants import BUDGET_CAPS_USD
from core.contracts.schemas import TerminationReason

from tests.integration._phase48_golden_helpers import (
    event_types_emitted,
    make_backtest_result,
    make_build_report,
    make_code_module,
    make_critic_verdict,
    make_strategy_spec,
    wire_orchestrator_for_golden,
)


def test_golden_budget_exceeded_loop(
    valid_hypothesis_id,
    mongo_test_db,
    frozen_registry_snapshot_hash,
    monkeypatch,
):
    from core.agents.refinement_orchestrator import main_loop as _ml

    original_governor = _ml.BudgetGovernor

    class _AlmostExhaustedGovernor(original_governor):
        def __init__(self, state):
            super().__init__(state)
            # Push loop_total_usd close enough to per_loop cap that preflight()
            # (which checks `remaining >= per_iteration`) returns False.
            self._budget_snapshot["loop_total_usd"] = (
                BUDGET_CAPS_USD["per_loop"] - 0.10
            )  # remaining = 0.10 < per_iteration (1.50)

    monkeypatch.setattr(
        "core.agents.refinement_orchestrator.main_loop.BudgetGovernor",
        _AlmostExhaustedGovernor,
    )

    seen_events: list[dict] = []
    mocks = wire_orchestrator_for_golden(
        monkeypatch,
        db=mongo_test_db,
        snapshot_hash=frozen_registry_snapshot_hash,
        run_strategy_returns=[make_strategy_spec()],
        run_code_returns=[make_code_module()],
        run_build_returns=[make_build_report()],
        run_backtest_returns=[make_backtest_result()],
        run_critic_returns=[make_critic_verdict()],  # never called — preflight terminates first
        seen_events=seen_events,
    )

    outcome = run_refinement_loop(valid_hypothesis_id)

    assert outcome.termination_reason == TerminationReason.BUDGET_EXCEEDED_LOOP
    # Critic should NEVER be reached when preflight terminates.
    assert mocks["run_critic"].call_count == 0

    emitted = event_types_emitted(seen_events)
    budget_events = [e for e in seen_events if e["event_type"] == "budget_exceeded"]
    assert budget_events, "budget_exceeded must be emitted"
    assert budget_events[-1]["payload"]["scope"] == "LOOP"
    assert "strategy_rejected" in emitted
    assert "loop_terminated" in emitted

    rejected = list(mongo_test_db["rejected_strategies"].find({}))
    assert rejected[0]["termination_reason"] == "BUDGET_EXCEEDED_LOOP"
